...
sub configureAPKSigningKeystore() {
    my ($self, $keystorePath, $keystorePassword, $alias, $keyPassword) = @_;
    my $response = $self->execute("configureAPKSigningKeystore", RPC::XML::string->new($keystorePath), RPC::XML::string->new($keystorePassword), RPC::XML::string->new($alias), RPC::XML::string->new($keyPassword));
}
