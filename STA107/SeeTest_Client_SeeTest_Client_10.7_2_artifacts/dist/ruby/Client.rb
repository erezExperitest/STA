	...
	class Client
        ...
        def execute(method, *params)
            unless (@clientMonitor.isRunning) or (@clientID == nil)
                puts("Error:")
                puts("The client has already been released. Please create a new client")
                return "ERROR"
            end
      ...
      def activateRecovery(typeXpathObj, method, *params)
          ...
          begin
            ...
            if e.is_a?(NotImplementedError)
              warn 'ERROR : Your Mobile Listener must implement the recover() method'
              raise e
            else
              warn 'ERROR : Listener Recover Method have thrown an exception with message ' + ge.message
              raise ge
            end
          end

          if not continueExe.nil? and continueExe
            warn "Successful Recovery attempt, Will try again the original method"
...
#           params.collect { |x| print x.to_s " " }
            ...
            warn "Unsuccessful Recovery attempt for Mobile Listener : " + typeXpathObj
            warn "Make sure that your recovery method return TRUE if passed"
            return "ERROR"
          end
        end
      ...
      def addMobileListener(elementType, xpath, listener)
        unless listener.nil?
          puts 'Adding mobile listener for XPath : ' + xpath + ' From type : ' + elementType
    ...
    begin
      ...
      puts e.faultString
        ...
        def configureAPKSigningKeystore(keystorePath, keystorePassword, alias, keyPassword)
            execute("configureAPKSigningKeystore", keystorePath, keystorePassword, alias, keyPassword)
        end
