...
#define	IC_MAX_MESSAGE_NUMBER 10000
	...
	*/
	...
	/// <remarks> On error or internal exception, you can look up for the error description under client.latest_result.details.internal_exception.message </remarks>
	*/
	static void ic_configure_a_p_k_signing_keystore(image_client *client /* A pointer to an 'image_client' handler (may not be NULL!) */,
	                                                const char* keystore_path /* Keystore file location on disk (default="") */,
	                                                const char* keystore_password /* Keystore password (default="") */,
	                                                const char* alias /* Alias (default="") */,
	                                                const char* key_password /* Store password (default="") */)
	{
		IC_API void ic_impl_configure_a_p_k_signing_keystore(image_client *client, ic_log *log, const char* keystore_path, const char* keystore_password, const char* alias, const char* key_password);
		ic_log log = { 0 };
		ic_impl_configure_a_p_k_signing_keystore(client, &log, keystore_path, keystore_password, alias, key_password);
		ic_output_log(&log);
		ic_process_exception(client);
	}
